<template>
<div :disabled="disabled" class="radio-list">
  <label v-for="item in items" :class="{'is-checked': isItemChecked(item)}">
    <input :id="getFieldID(schema)" type="radio" :disabled="disabled" :name="id" @click="onSelection(item)" :value="getItemValue(item)" :checked="isItemChecked(item)" :class="schema.fieldClasses"/>{{ getItemName(item) }}
  </label>
</div>
</template>
