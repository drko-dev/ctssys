...
<div class="wrapper">
  <div v-if="schema.listBox" :disabled="disabled" class="listbox form-control">
    <div v-for="item in items" :class="{'is-checked': isItemChecked(item)}" class="list-row">
      <label>
        <input :id="getFieldID(schema)" type="checkbox" :checked="isItemChecked(item)" :disabled="disabled" @change="onChanged($event, item)" :name="getInputName(item)"/>{{ getItemName(item) }}
      </label>
    </div>
  </div>
